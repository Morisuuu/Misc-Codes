...
knn_anomaly_detection <- function(data, column1, column2, k = 5, threshold = 2) {
  tryCatch({
    ...
    print(e)
    return(list(anomalies = NULL, anomaly_scores = NULL, normal_kde = NULL, anomaly_kde = NULL))
  })
  if (is.null(model)) {
  ...
  normal <- data[anomaly_score <= threshold, ]
  
  # Calculate KDE for normal data
  normal_kde <- density(normal[[column1]])
  
  # Calculate KDE for anomalous data
  anomaly_kde <- density(anomalies[[column1]])
  
  return(list(anomalies = anomalies, anomaly_scores = anomaly_score, normal_kde = normal_kde, anomaly_kde = anomaly_kde))
}
...
# Plot KDE
ggplot() +
  geom_line(data = data.frame(x = result$normal_kde$x, y = result$normal_kde$y), aes(x, y, color = "Normal"), size = 1) +
  geom_line(data = data.frame(x = result$anomaly_kde$x, y = result$anomaly_kde$y), aes(x, y, color = "Anomaly"), size = 1) +
  labs(x = "AVE.TMP", y = "Density", title = "KDE of K-NN Anomalies",
       color = "Legend") + # Change "colour" to "legend"
...
d <- ggplot(data, aes(x=`AVE.TEMP`, color=)) + 
  geom_density()
print(d)
